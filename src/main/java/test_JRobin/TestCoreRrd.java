...
//			rrdDef.addArchive("AVERAGE", 0.5, 6, 5);
//			rrdDef.addArchive("AVERAGE", 0.5, 24, 5);
	...
	private void createRRDInitData(long startTime, long endTime,
		...
		try {
			...
			for (long t = startTime; t < endTime; t += 300) {
				...
				if(num++ > 10){
					sample.setValue("input", 112);
					sample.setValue("output", 113);
//					sample.update();
					break;
				}
...
//				sample.update();
				...
				sample.setValue("input", num);
				sample.setValue("output", num);
				sample.update();
				System.out.println(num);
//				System.out.println(tmpval + 50 + " ===== " + tmpval2 + 50);	
			}
