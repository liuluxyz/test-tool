 ...
 */
public class MYSQLConnHelper2 {

//	private static final Log LOG = LogFactory.getLog(MYSQLConnHelper2.class);
...
//	private static final String url = "jdbc:mysql://localhost/ctetl";
	private static final String url = "jdbc:mysql://172.16.2.103:3306/information_schema";
	private static final String user = "ocetl";
	private static final String password = "ocetl";
	
	private static MYSQLConnHelper2 mysqlConnHelper;
	
	private MYSQLConnHelper2(){
		init();
	}
	
	public static MYSQLConnHelper2 getInstance(){
		if(mysqlConnHelper == null){
			mysqlConnHelper = new MYSQLConnHelper2();
		}
...
//			LOG.error("mysqlConnHelper.close", e);
	...
	private static int getTableRowCount(String sql){
		Connection conn = MYSQLConnHelper2.getInstance().getConnection();
		...
		try {
			...
			MYSQLConnHelper2.getInstance().close(conn, state, set);
		}
		return 0;
	}
...
//				break;
			...
			MYSQLConnHelper2.getInstance().close(stat, result);
			MYSQLConnHelper2.getInstance().close(conn);
		}
	}
